...
| Operational controls | Implemented; activation in progress | CloudWatch alarms/dashboard, encrypted SNS alerts, SQS DLQs, EventBridge failure handling, and runbooks. The dev email recipient must still confirm SNS delivery. |
...
### Live dev activation snapshot — 2026-07-24
...
- The encrypted SNS topic `streamforge-dev-alerts` has an email subscription
  for `kumarakshat868@gmail.com`. It was **PendingConfirmation** on 2026-07-24.
  The mailbox owner must confirm the AWS email before alerts can be delivered;
  never treat a pending subscription as an operationally tested alert path.
...
- GitHub `dev` currently has no required-reviewer protection rule. Do not claim
  a manual approval gate exists until the owner supplies an independent GitHub
  user or team and that rule has been configured. Production is intentionally
  not activated because no separate production AWS account/role is available.
...
| Amazon SNS | Operational alert notifications. | Alert topic is encrypted. The dev email subscription is created but awaiting recipient confirmation before it can deliver alerts. |
...
1. Confirm the `streamforge-dev-alerts` SNS email subscription, then publish a
   safe test notification and exercise the applicable CloudWatch alarm paths.
