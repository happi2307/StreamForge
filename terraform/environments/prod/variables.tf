...
variable "alerts_sns_kms_master_key_id" {
  description = "KMS key id or alias used to encrypt the alerts SNS topic. Null leaves it unencrypted."
  type        = string
  default     = null
}
...
variable "glue_failure_rule_name" {
  description = "Name of the EventBridge rule that captures Phase 3 Glue job failures."
  type        = string
  default     = "streamforge-glue-job-failed"
}

variable "glue_failure_states" {
  description = "Glue job states treated as failures for alerting."
  type        = list(string)
